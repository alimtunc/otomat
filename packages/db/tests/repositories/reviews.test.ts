...
import { getReviewForSubject, insertReview, updateReviewStatus } from "#db/repositories/reviews";
...
it("round-trips a review and its pin-to-SHA comments for a run", () => {
  const { db } = t.client;
  insertReview(db, { id: "rv1", subject_id: "r1", status: "open" });
  expect(getReviewForSubject(db, "r1")?.status).toBe("open");
  ...
  expect(listReviewCommentsForSubject(db, "r1").map((c) => c.id)).toEqual(["c1"]);
  ...
  expect(getReviewForSubject(db, "r1")?.status).toBe("in_review");
});
