...
import type { RuntimeEvent } from "@otomat/runtime";
...
/**
 * Builds a migrated temp DB with the full parent chain (project -> issue -> run
 * -> step_run -> agent_session) so `runtime_events` foreign keys are satisfied,
 * plus a scratch dir for the run's `events.jsonl`.
 */
