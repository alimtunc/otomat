...
test("ships the seeder and runs it behind the daemon it fills", () => {
  assert.match(dockerfile, /COPY seed\.mjs \/srv\/seed\.mjs/);
  const start = readFileSync(new URL("./start.sh", import.meta.url), "utf8");
  assert.match(start, /node \/srv\/seed\.mjs &/);
  assert.match(start, /exec node \/srv\/daemon\/dist\/index\.js/);
});
