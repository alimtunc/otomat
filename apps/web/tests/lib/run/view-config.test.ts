...
describe("runs view config", () => {
  it("hides done issues and keeps failed runs by default", () => {
    expect(DEFAULT_RUNS_VIEW_CONFIG).toEqual({ showFailed: true, showClosedIssues: false });
  ...
  it("restores a project's filters without reaching another project", () => {
    ...
    writeRunsViewConfig("project-1", { showFailed: false, showClosedIssues: true }, storage);
