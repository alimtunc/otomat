...
describe("visibleRunGroups", () => {
  ...
  it("hides a canceled issue's group too, review-ready run and all", () => {
    const canceled = groupRunsByIssue(
      [runContract({ id: "r4", issue_id: "issue-3", status: "review_ready" })],
      [issueContract({ id: "issue-3", title: "Dropped", status: "canceled" })],
    );

    expect(visibleRunGroups(canceled, DEFAULT_RUNS_VIEW_CONFIG)).toMatchObject({
      groups: [],
      hiddenGroups: 1,
    });
  });
  ...
  it("keeps failed runs until they are switched off", () => {
    ...
    const hidden = visibleRunGroups(groups, { showFailed: false, showClosedIssues: false });
  ...
  it("shows everything once both axes are on", () => {
    const visible = visibleRunGroups(groups, { showFailed: true, showClosedIssues: true });
  ...
  it("counts a group emptied by the failed filter as its runs, not as a hidden issue", () => {
    ...
    const visible = visibleRunGroups(onlyFailed, { showFailed: false, showClosedIssues: true });
  ...
  it("leaves the runs themselves untouched", () => {
    visibleRunGroups(groups, { showFailed: false, showClosedIssues: false });
