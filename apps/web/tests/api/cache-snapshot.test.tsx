...
function seeded(issues: IssueContract[], updatedAt: number): QueryClient {
  ...
  source.setQueryData(queryKeys.issuesList("p1"), issues, { updatedAt });
  return source;
}

function listedIssues(cache: QueryClient): IssueContract[] | undefined {
  return cache.getQueryData(queryKeys.issuesList("p1"));
}

function Probe() {
  const query = useQuery<IssueContract[]>({
    queryKey: queryKeys.issuesList("p1"),
...
it("restores the lists a visited project was last showing, still marked as of their fetch", () => {
  ...
  expect(restored.getQueryState(queryKeys.issuesList("p1"))?.dataUpdatedAt).toBe(fetchedAt);
});
...
it("revalidates a restored list even when it is younger than the stale time", () => {
  ...
  expect(restored.getQueryState(queryKeys.issuesList("p1"))?.isInvalidated).toBe(true);
});
...
it("never serves one host's snapshot to another", () => {
  const storage = memoryStorage();
...
it("stores what a tab reopens on, but never a diff or the live host status", () => {
  ...
  const diffKey = queryKeys.reviewDiff({ kind: "run", id: "run-1" });
  source.setQueryData(diffKey, { files: [] });
  source.setQueryData(queryKeys.issue("i1"), issueContract({ id: "i1" }));
  source.setQueryData(queryKeys.executionHost, { active_id: "local" });
  ...
  expect(restored.getQueryData(queryKeys.issue("i1"))).toBeDefined();
  expect(restored.getQueryData(diffKey)).toBeUndefined();
  expect(restored.getQueryData(queryKeys.executionHost)).toBeUndefined();
});
...
it("keeps writing while the window lives, so a crash loses at most one interval", () => {
  ...
  attached.setQueryData(queryKeys.issuesList("p1"), [issueContract({ id: "i1" })]);
...
it("restores on attach and writes the snapshot back when the document goes away", () => {
  ...
  attached.setQueryData(queryKeys.issuesList("p1"), [issueContract({ id: "i2" })]);
