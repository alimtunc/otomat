import type { LinearDeliverySnapshot, OtomatDesktopBridge } from "@otomat/domain";
...
/**
 * Where the vaulted Linear key stands on every execution host. Null in the browser,
 * which has no vault: there the daemon it was started against holds its own key.
 */
export function useLinearDelivery(): LinearDeliverySnapshot | null {
  ...
  const delivery = useQuery({
    ...
    queryFn: () => requireBridge(bridge).linear.delivery(),
    enabled: bridge !== null,
  });
