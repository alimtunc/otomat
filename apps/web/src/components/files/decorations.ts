...
export interface DecoratedFile extends WorktreeFileEntry {
  status?: ChangeStatus;
}
...
export function decorateFiles(
  ...
  const files = new Map<string, DecoratedFile>(entries.map((entry) => [entry.path, { ...entry }]));
  for (const change of [...(changes?.staged ?? []), ...(changes?.unstaged ?? [])]) {
    ...
    if (entry !== undefined) files.set(change.path, { ...entry, status });
      ...
      files.set(change.path, { path: change.path, kind: "file", size: 0, status });
  }
