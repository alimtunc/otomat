...
export function UsageDetail({ event }: { event: EventEnvelope }) {
  ...
  if (usage.inputTokens !== null) {
    ...
    exactParts.push(`in ${formatExactTokenCount(usage.inputTokens)}`);
  }
  if (usage.outputTokens !== null) {
    parts.push(`out ${formatTokenCount(usage.outputTokens)}`);
    exactParts.push(`out ${formatExactTokenCount(usage.outputTokens)}`);
  }
  ...
  return (
    <span
      className="mt-1 inline-flex items-center rounded-full border border-border-subtle bg-surface-1 px-2.5 py-0.5 font-mono text-xs text-text-secondary"
