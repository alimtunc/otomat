...
export function deployDaemonScript(options: DeployDaemonScriptOptions): string {
  ...
  return [
    ...
    "  exit 0",
    "fi",
    `ID="$(gh api "repos/${options.repo}/actions/artifacts?name=$NAME&per_page=1" --jq '.artifacts[0].id' 2>/dev/null || true)"`,
    ...
    'mv "$TMP/x/daemon" "$OTOMAT_HOME/daemon.next"',
    'if [ -d "$OTOMAT_HOME/daemon" ]; then mv "$OTOMAT_HOME/daemon" "$OTOMAT_HOME/daemon.prev"; fi',
    'mv "$OTOMAT_HOME/daemon.next" "$OTOMAT_HOME/daemon"',
