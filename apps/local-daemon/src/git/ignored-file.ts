...
export async function readIgnoredFile(
  worktreePath: string,
  path: string,
  ...
  if (!stat.isFile() || !(await isIgnored(worktreePath, path))) return { kind: "missing" };
  if (stat.size > limits.maxBytes) return { kind: "too_large", bytes: stat.size };
  const content = readWithoutFollowing(target);
  const oid = await blobRevision(worktreePath, path, content);
