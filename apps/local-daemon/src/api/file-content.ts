...
export async function createEntryResponse(
  c: Context,
  cwd: string,
  ...
  const created = await createWorktreeEntry(cwd, { ...request, path });
  return typeof created === "string" ? refuseFile(c, created) : c.json(created, 201);
}

export async function saveFileResponse(c: Context, cwd: string, request: SaveWorktreeFileRequest) {
  const path = normalizeRepositoryPath(request.path);
  ...
  const result = await writeWorktreeFile(cwd, path, request.revision, request.text);
  switch (result.kind) {
    case "written":
...
export async function fileContentResponse(c: Context, tree: WorktreeTree, path: string) {
  const mediaType = diffMediaTypeForPath(path);
  const captured = await tree.readFile(path, {
  ...
  const read =
    live === null
      ? captured
      : await readIgnoredFile(live, path, { maxBytes: WORKTREE_FILE_MAX_BYTES });
  switch (read.kind) {
    case "text":
      return c.json({
        kind: "text",
    ...
    case "binary":
      ...
      return c.json({
        ...
        revision: read.oid,
        bytes: read.bytes,
        data: (await tree.readBlob(read.oid)).toString("base64"),
