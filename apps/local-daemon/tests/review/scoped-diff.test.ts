import { execFileSync } from "node:child_process";
import { unlinkSync, writeFileSync } from "node:fs";
import { join } from "node:path";

import {
  getRun,
  insertAgentSession,
  insertPullRequest,
  type NewPullRequest,
  recordSessionBoundaryError,
  recordSessionPassEnd,
  recordSessionPassStart,
  type RunRow,
} from "@otomat/db";
import { BRANCH_DIFF_SCOPE, DEFAULT_DIFF_SCOPE, type RunDiffScopeSelector } from "@otomat/domain";
import { afterEach, beforeEach, expect, it } from "vitest";

import { createRepositoryResolver, type GitWorktreeService } from "#git";
import { createReviewService, DiffScopeNotFoundError, type ReviewService } from "#review";

import { setupDaemonDb, type DaemonTestDb } from "../support/daemon-db.js";
import { seedRun, seedWorkflowRun } from "../support/seed.js";

const RUN_ID = "r-scope";
const SESSION_ID = `${RUN_ID}-session`;
const STEP_ID = `${RUN_ID}-step`;
const BRANCH = "otomat/run/r-scope";
const TARGET = { kind: "run", id: RUN_ID } as const;
const PASS: RunDiffScopeSelector = { kind: "session", session: SESSION_ID };
const STEP: RunDiffScopeSelector = { kind: "step", step: STEP_ID };
const PULL_REQUEST: RunDiffScopeSelector = { kind: "pull_request" };

let fix: DaemonTestDb;
let worktrees: GitWorktreeService;
let review: ReviewService;
let worktreePath = "";

function gitIn(cwd: string, ...args: string[]): string {
  return execFileSync("git", args, { cwd, encoding: "utf8" }).trim();
}

function git(...args: string[]): string {
  return gitIn(worktreePath, ...args);
}

function run(): RunRow {
  const row = getRun(fix.db, RUN_ID);
  if (!row) throw new Error("seeded run missing");
  return row;
}

/** Capture the pass boundary exactly as the supervisor does around a turn. */
async function capture(end: boolean, sessionId = SESSION_ID, runId = RUN_ID): Promise<void> {
  const state = await worktrees.captureState(runId);
  if (end) recordSessionPassEnd(fix.db, sessionId, state);
  else recordSessionPassStart(fix.db, sessionId, state);
}

function addTurn(id: string): string {
  insertAgentSession(fix.db, { id, step_run_id: STEP_ID, status: "terminated" });
  return id;
}

function commit(message: string): string {
  git("add", "-A");
  git("commit", "-m", message);
  return git("rev-parse", "HEAD");
}

const insertRunPullRequest = (
  pr: Pick<NewPullRequest, "id" | "number" | "base_ref" | "published_head_sha">,
): void => {
  insertPullRequest(fix.db, {
    issue_id: "i1",
    run_id: RUN_ID,
    repository_id: "repo-1",
    status: "open",
    publication_status: "created",
    title: "Feature",
    head_ref: BRANCH,
    ...pr,
  });
};

beforeEach(async () => {
  fix = setupDaemonDb();
  const repositories = createRepositoryResolver({
    db: fix.db,
    worktreesRoot: join(fix.dataDir, "worktrees"),
  });
  const binding = repositories.forRepository("repo-1");
  if (!binding) throw new Error("repo-1 binding missing");
  worktrees = binding.service;
  review = createReviewService({
    db: fix.db,
    dataDir: fix.dataDir,
    repositories,
    appendRunStep: async () => run(),
    submitPullRequestReview: async () => ({ url: "https://example.invalid/1" }),
    syncViewedFile: async () => "octocat",
    readViewedFiles: async () => ({ viewerLogin: "octocat", files: [] }),
  });

  const acquired = await worktrees.acquire({ owner: RUN_ID, branch: BRANCH });
  worktreePath = acquired.path;
  git("config", "user.email", "t@t");
  git("config", "user.name", "t");
  seedRun(fix.db, {
    runId: RUN_ID,
    repositoryId: "repo-1",
    worktreeId: acquired.id,
    runStatus: "review_ready",
    stepStatus: "succeeded",
    sessionStatus: "terminated",
  });
});

afterEach(() => {
  fix.cleanup();
});

it("reconstructs a pass that produced only uncommitted changes", async () => {
  await capture(false);
  writeFileSync(join(worktreePath, "notes.md"), "alpha\n");
  await capture(true);

  const result = await review.getDiff(TARGET, PASS);

  expect(result.unavailable).toBeNull();
  expect(result.diff?.files.map((file) => file.path)).toEqual(["notes.md"]);
  expect(result.scope).toMatchObject({ kind: "session", agent_session_id: SESSION_ID });
});

it("keeps a pass delta stable after a later pass changes the same file", async () => {
  await capture(false);
  writeFileSync(join(worktreePath, "notes.md"), "alpha\n");
  await capture(true);
  const before = (await review.getDiff(TARGET, PASS)).diff;

  writeFileSync(join(worktreePath, "notes.md"), "alpha\nbeta\ngamma\n");
  writeFileSync(join(worktreePath, "other.md"), "later work\n");

  const after = (await review.getDiff(TARGET, PASS)).diff;
  expect(after?.sha).toBe(before?.sha);
  expect(after?.files.map((file) => file.path)).toEqual(["notes.md"]);
  // The branch has moved on; the pass has not.
  expect((await review.getDiff(TARGET, BRANCH_DIFF_SCOPE)).diff?.files.map((f) => f.path)).toEqual([
    "notes.md",
    "other.md",
  ]);
});

it("includes a pass's commits as well as the work it left uncommitted", async () => {
  await capture(false);
  writeFileSync(join(worktreePath, "committed.md"), "in a commit\n");
  commit("step work");
  writeFileSync(join(worktreePath, "dirty.md"), "not committed\n");
  await capture(true);

  const files = (await review.getDiff(TARGET, PASS)).diff?.files.map((file) => file.path);
  expect(files).toEqual(["committed.md", "dirty.md"]);
});

it("reports a pass with no end boundary rather than showing the branch diff", async () => {
  await capture(false);
  writeFileSync(join(worktreePath, "notes.md"), "alpha\n");

  const result = await review.getDiff(TARGET, PASS);

  expect(result.diff).toBeNull();
  expect(result.unavailable).toContain("not finished");
});

it("keeps a boundary failure verbatim instead of inventing a delta", async () => {
  recordSessionBoundaryError(fix.db, SESSION_ID, "the worktree vanished mid-turn");

  const result = await review.getDiff(TARGET, PASS);

  expect(result.diff).toBeNull();
  expect(result.unavailable).toBe("the worktree vanished mid-turn");
});

it("says so when git no longer holds a captured boundary tree", async () => {
  await capture(false);
  writeFileSync(join(worktreePath, "notes.md"), "alpha\n");
  await capture(true);
  recordSessionPassEnd(fix.db, SESSION_ID, {
    treeSha: "0".repeat(40),
    headSha: "0".repeat(40),
  });

  const result = await review.getDiff(TARGET, PASS);

  expect(result.diff).toBeNull();
  expect(result.unavailable).toContain("no longer holds");
});

it("diffs a chosen commit against its parent, not against the branch fork point", async () => {
  writeFileSync(join(worktreePath, "first.md"), "one\n");
  const first = commit("first");
  writeFileSync(join(worktreePath, "second.md"), "two\n");
  const second = commit("second");

  expect(
    (await review.getDiff(TARGET, { kind: "commit", commit: first })).diff?.files.map(
      (f) => f.path,
    ),
  ).toEqual(["first.md"]);
  const result = await review.getDiff(TARGET, { kind: "commit", commit: second });

  expect(result.diff?.files.map((file) => file.path)).toEqual(["second.md"]);
  expect(result.scope).toMatchObject({ kind: "commit", subject: "second" });
  // The whole branch carries both; the commit scope must not be confused with it.
  expect((await review.getDiff(TARGET, BRANCH_DIFF_SCOPE)).diff?.files.map((f) => f.path)).toEqual([
    "first.md",
    "second.md",
  ]);
});

it("lists the branch commits with the identity a picker needs", async () => {
  writeFileSync(join(worktreePath, "first.md"), "one\n");
  git("add", "-A");
  git("-c", "user.email=picker@t", "-c", "user.name=Picker", "commit", "-m", "first");

  const listed = await review.getBranchCommits(RUN_ID);

  expect(listed.unavailable).toBeNull();
  expect(listed.commits).toHaveLength(1);
  expect(listed.commits[0]).toMatchObject({ subject: "first", author_name: "Picker" });
  expect(listed.commits[0]?.short_sha).toHaveLength(7);
});

it("refuses an unknown commit and an unknown pass rather than answering with another scope", async () => {
  await expect(review.getDiff(TARGET, { kind: "commit", commit: "f".repeat(40) })).rejects.toThrow(
    DiffScopeNotFoundError,
  );
  await expect(review.getDiff(TARGET, { kind: "session", session: "nope" })).rejects.toThrow(
    DiffScopeNotFoundError,
  );
});

it("expands context against the trees the scope was taken between", async () => {
  writeFileSync(join(worktreePath, "notes.md"), "alpha\n");
  await capture(false);
  writeFileSync(join(worktreePath, "notes.md"), "alpha\nbeta\n");
  await capture(true);
  // The branch moves on after the pass; expansion must not follow it.
  writeFileSync(join(worktreePath, "notes.md"), "alpha\nbeta\ngamma\n");

  const file = (await review.getDiff(TARGET, PASS)).diff?.files.find((f) => f.path === "notes.md");
  if (!file) throw new Error("expected notes.md in the pass delta");
  const blobs = await review.getFileBlobs(TARGET, { path: "notes.md", sha: file.sha, scope: PASS });

  expect(blobs.base).toEqual({ kind: "text", content: "alpha\n" });
  expect(blobs.head).toEqual({ kind: "text", content: "alpha\nbeta\n" });
});

it("reports a run without a repository instead of throwing", async () => {
  seedRun(fix.db, {
    runId: "r-bare",
    repositoryId: null,
    runStatus: "review_ready",
    stepStatus: "succeeded",
    sessionStatus: "terminated",
  });
  const bare = getRun(fix.db, "r-bare");
  if (!bare) throw new Error("bare run missing");

  const result = await review.getDiff({ kind: "run", id: bare.id }, BRANCH_DIFF_SCOPE);

  expect(result.diff).toBeNull();
  expect(result.unavailable).toContain("no git repository");
  expect((await review.getBranchCommits("r-bare")).unavailable).toContain("no git repository");
});

it("reports a removed worktree rather than reporting an empty branch", async () => {
  writeFileSync(join(worktreePath, "notes.md"), "alpha\n");
  await capture(false);
  unlinkSync(join(worktreePath, "notes.md"));
  await worktrees.cleanup(RUN_ID);

  expect((await review.getBranchCommits(RUN_ID)).unavailable).toContain("worktree is gone");
});

it("spans every turn of a step, from the snapshot it entered on to the one it left", async () => {
  await capture(false);
  writeFileSync(join(worktreePath, "first-turn.md"), "one\n");
  await capture(true);
  const resumed = addTurn(`${STEP_ID}-turn-2`);
  await capture(false, resumed);
  writeFileSync(join(worktreePath, "second-turn.md"), "two\n");
  await capture(true, resumed);

  const result = await review.getDiff(TARGET, STEP);

  expect(result.unavailable).toBeNull();
  expect(result.diff?.files.map((file) => file.path)).toEqual(["first-turn.md", "second-turn.md"]);
  expect(result.scope).toMatchObject({
    kind: "step",
    step_run_id: STEP_ID,
    step_name: "Agent turn",
    step_number: 1,
  });
  expect((await review.getDiff(TARGET, PASS)).diff?.files.map((f) => f.path)).toEqual([
    "first-turn.md",
  ]);
});

it("reports a step whose last turn has not left a snapshot instead of showing the branch diff", async () => {
  await capture(false);
  writeFileSync(join(worktreePath, "done.md"), "one\n");
  await capture(true);
  const resumed = addTurn(`${STEP_ID}-turn-2`);
  await capture(false, resumed);
  writeFileSync(join(worktreePath, "in-flight.md"), "two\n");

  const result = await review.getDiff(TARGET, STEP);

  expect(result.diff).toBeNull();
  expect(result.unavailable).toContain("not finished");
  expect(result.scope).toMatchObject({ kind: "step", step_run_id: STEP_ID });
});

it("gives each successive step of a run only its own changes", async () => {
  const runId = "r-steps";
  const acquired = await worktrees.acquire({ owner: runId, branch: `otomat/run/${runId}` });
  seedWorkflowRun(fix.db, {
    runId,
    repositoryId: "repo-1",
    worktreeId: acquired.id,
    runStatus: "review_ready",
    steps: [
      { id: "s-plan", name: "Plan", status: "succeeded", session: { status: "terminated" } },
      { id: "s-fix", name: "Fix", status: "succeeded", session: { status: "terminated" } },
    ],
  });
  await capture(false, "s-plan-session", runId);
  writeFileSync(join(acquired.path, "plan.md"), "planned\n");
  await capture(true, "s-plan-session", runId);
  await capture(false, "s-fix-session", runId);
  writeFileSync(join(acquired.path, "fix.md"), "fixed\n");
  await capture(true, "s-fix-session", runId);

  const target = { kind: "run", id: runId } as const;
  const plan = await review.getDiff(target, { kind: "step", step: "s-plan" });
  const fixStep = await review.getDiff(target, { kind: "step", step: "s-fix" });

  expect(plan.diff?.files.map((file) => file.path)).toEqual(["plan.md"]);
  expect(plan.scope).toMatchObject({ step_name: "Plan", step_number: 1 });
  expect(fixStep.diff?.files.map((file) => file.path)).toEqual(["fix.md"]);
  expect(fixStep.scope).toMatchObject({ step_name: "Fix", step_number: 2 });
  expect((await review.getDiff(target, BRANCH_DIFF_SCOPE)).diff?.files.map((f) => f.path)).toEqual([
    "fix.md",
    "plan.md",
  ]);
});

it("lists a path changed more than once exactly once in every scope that carries it", async () => {
  await capture(false);
  writeFileSync(join(worktreePath, "shared.md"), "one\n");
  await capture(true);
  const resumed = addTurn(`${STEP_ID}-turn-2`);
  await capture(false, resumed);
  writeFileSync(join(worktreePath, "shared.md"), "one\ntwo\n");
  await capture(true, resumed);

  const step = (await review.getDiff(TARGET, STEP)).diff;
  const later = (await review.getDiff(TARGET, { kind: "session", session: resumed })).diff;
  const branchDiff = (await review.getDiff(TARGET, BRANCH_DIFF_SCOPE)).diff;

  for (const diff of [step, later, branchDiff]) {
    expect(diff?.files.map((file) => file.path)).toEqual(["shared.md"]);
  }
});

it("keeps the branch diff to the branch's own work after a rebase moves its fork point", async () => {
  writeFileSync(join(worktreePath, "feature.md"), "branch work\n");
  commit("feature");
  fix.repo.write("base-move.md", "moved on\n");
  fix.repo.commitAll("base branch moves on");
  git("rebase", "main");

  const branchDiff = await review.getDiff(TARGET, BRANCH_DIFF_SCOPE);

  expect(branchDiff.diff?.files.map((file) => file.path)).toEqual(["feature.md"]);
  expect((await review.getBranchCommits(RUN_ID)).commits.map((c) => c.subject)).toEqual([
    "feature",
  ]);
});

it("still shows a merged cycle's own work once its base branch contains it", async () => {
  writeFileSync(join(worktreePath, "feature.md"), "branch work\n");
  commit("feature");
  fix.repo.git("merge", "--no-ff", "-m", "merge", BRANCH);

  const branchDiff = await review.getDiff(TARGET, BRANCH_DIFF_SCOPE);

  expect(branchDiff.diff?.files.map((file) => file.path)).toEqual(["feature.md"]);
  expect((await review.getBranchCommits(RUN_ID)).commits.map((c) => c.subject)).toEqual([
    "feature",
  ]);
});

it("shows the published head against the branch it targets, not everything the branch holds", async () => {
  writeFileSync(join(worktreePath, "feature.md"), "branch work\n");
  commit("feature");
  fix.repo.write("base-move.md", "moved on\n");
  fix.repo.commitAll("base branch moves on");
  git("rebase", "main");
  const published = git("rev-parse", "HEAD");
  insertRunPullRequest({
    id: "pr-scope",
    number: 79,
    base_ref: "main",
    published_head_sha: published,
  });

  const result = await review.getDiff(TARGET, PULL_REQUEST);

  expect(result.unavailable).toBeNull();
  expect(result.diff?.files.map((file) => file.path)).toEqual(["feature.md"]);
  expect(result.scope).toEqual({ kind: "pull_request", number: 79 });
});

/** A clone whose `main` stayed where it was pulled while `origin/main` moved on. */
function advanceOriginPast(...unrelated: string[]): string {
  fix.repo.git("checkout", "--quiet", "-b", "upstream-work", "main");
  for (const path of unrelated) {
    fix.repo.write(path, `landed on main before the run forked: ${path}\n`);
    fix.repo.commitAll(`upstream: ${path}`);
  }
  fix.repo.git("push", "--quiet", "origin", "upstream-work:main");
  fix.repo.git("checkout", "--quiet", "main");
  fix.repo.git("fetch", "--quiet", "origin");
  return fix.repo.git("rev-parse", "origin/main").trim();
}

/** Worktree of a run acquired at the fetched remote tip, exactly as `resolveBaseSha` hands it to a launch. */
async function runForkedFromOrigin(runId: string, originTip: string): Promise<string> {
  const acquired = await worktrees.acquire({
    owner: runId,
    branch: `otomat/run/${runId}`,
    baseSha: originTip,
  });
  seedRun(fix.db, {
    runId,
    repositoryId: "repo-1",
    worktreeId: acquired.id,
    runStatus: "review_ready",
    stepStatus: "succeeded",
    sessionStatus: "terminated",
  });
  return acquired.path;
}

function commitFeatureOn(worktree: string): string {
  writeFileSync(join(worktree, "feature.md"), "the only file the PR carries\n");
  gitIn(worktree, "add", "-A");
  gitIn(worktree, "commit", "-m", "feature");
  return gitIn(worktree, "rev-parse", "HEAD");
}

function insertPullRequestForRun(runId: string, id: string, number: number, head: string): void {
  insertPullRequest(fix.db, {
    id,
    issue_id: "i1",
    run_id: runId,
    repository_id: "repo-1",
    status: "open",
    publication_status: "created",
    title: "Feature",
    head_ref: `otomat/run/${runId}`,
    number,
    base_ref: "main",
    published_head_sha: head,
  });
}

it("measures a pull request against the base the remote publishes, not against a local branch left behind", async () => {
  const originTip = advanceOriginPast("unrelated-a.md", "unrelated-b.md", "unrelated-c.md");
  const target = { kind: "run", id: "r-stale-pr" } as const;
  const forked = await runForkedFromOrigin(target.id, originTip);
  const published = commitFeatureOn(forked);
  insertPullRequestForRun(target.id, "pr-stale-base", 224, published);

  const pullRequest = await review.getDiff(target, PULL_REQUEST);
  const branchDiff = await review.getDiff(target, BRANCH_DIFF_SCOPE);

  expect(pullRequest.diff?.files.map((file) => file.path)).toEqual(["feature.md"]);
  expect(pullRequest.diff?.base).toBe(originTip);
  expect(branchDiff.diff?.files.map((file) => file.path)).toEqual(["feature.md"]);
  expect(branchDiff.diff?.base).toBe(originTip);
});

it("answers a pull request opened as its own subject with the same patch its run does", async () => {
  const originTip = advanceOriginPast("unrelated-a.md", "unrelated-b.md");
  const target = { kind: "run", id: "r-opened-pr" } as const;
  const published = commitFeatureOn(await runForkedFromOrigin(target.id, originTip));
  insertPullRequestForRun(target.id, "pr-opened", 224, published);

  const fromRun = await review.getDiff(target, PULL_REQUEST);
  const opened = await review.getDiff(
    { kind: "pull_request", id: "pr-opened" },
    DEFAULT_DIFF_SCOPE,
  );

  expect(opened.diff?.sha).toBe(fromRun.diff?.sha);
  expect(opened.diff?.files.map((file) => file.path)).toEqual(["feature.md"]);
});

it("defaults a run to its pull request, and to its branch when it has none", async () => {
  writeFileSync(join(worktreePath, "uncommitted.md"), "still in the worktree\n");
  commit("feature");

  expect((await review.getDiff(TARGET, DEFAULT_DIFF_SCOPE)).scope).toMatchObject({
    kind: "branch",
  });

  insertRunPullRequest({
    id: "pr-default",
    number: 82,
    base_ref: "main",
    published_head_sha: git("rev-parse", "HEAD"),
  });

  const defaulted = await review.getDiff(TARGET, DEFAULT_DIFF_SCOPE);

  expect(defaulted.scope).toEqual({ kind: "pull_request", number: 82 });
  expect(defaulted.diff?.files.map((file) => file.path)).toEqual(["uncommitted.md"]);
});

it("keeps an explicitly chosen branch as its own answer, never re-read as the default", async () => {
  writeFileSync(join(worktreePath, "uncommitted.md"), "still in the worktree\n");
  commit("feature");
  insertRunPullRequest({
    id: "pr-explicit",
    number: 83,
    base_ref: "main",
    published_head_sha: git("rev-parse", "HEAD"),
  });

  expect((await review.getDiff(TARGET, BRANCH_DIFF_SCOPE)).scope).toMatchObject({ kind: "branch" });
});

it("defaults to the branch when a publication left a pull request row with no head", async () => {
  writeFileSync(join(worktreePath, "uncommitted.md"), "still in the worktree\n");
  commit("feature");
  insertRunPullRequest({ id: "pr-unpublished-default", number: null, base_ref: "main" });

  const defaulted = await review.getDiff(TARGET, DEFAULT_DIFF_SCOPE);

  expect(defaulted.scope).toMatchObject({ kind: "branch" });
  expect(defaulted.diff?.files.map((file) => file.path)).toEqual(["uncommitted.md"]);
});

it("freezes the default at the published head while the branch moves past it", async () => {
  writeFileSync(join(worktreePath, "published.md"), "in the pull request\n");
  const published = commit("published");
  insertRunPullRequest({
    id: "pr-moved-on",
    number: 84,
    base_ref: "main",
    published_head_sha: published,
  });
  writeFileSync(join(worktreePath, "after.md"), "landed after the push\n");
  commit("after the push");

  const defaulted = await review.getDiff(TARGET, DEFAULT_DIFF_SCOPE);
  const branchDiff = await review.getDiff(TARGET, BRANCH_DIFF_SCOPE);

  expect(defaulted.scope).toEqual({ kind: "pull_request", number: 84 });
  expect(defaulted.diff?.files.map((file) => file.path)).toEqual(["published.md"]);
  expect(branchDiff.diff?.files.map((file) => file.path)).toEqual(["after.md", "published.md"]);

  const file = defaulted.diff?.files[0];
  if (file === undefined) throw new Error("the default patch carried no file");
  const comment = await review.addComment(TARGET, {
    file_path: file.path,
    diff_sha: file.sha,
    side: "new",
    line: 1,
    body: "from the default view",
    destination: "agent",
  });

  expect(comment.diff_sha).toBe(file.sha);
});

it("names an unpublished pull request as such rather than answering with its branch diff", async () => {
  writeFileSync(join(worktreePath, "notes.md"), "alpha\n");
  insertRunPullRequest({ id: "pr-unpublished", number: 80, base_ref: "main" });

  const result = await review.getDiff(TARGET, PULL_REQUEST);

  expect(result.diff).toBeNull();
  expect(result.unavailable).toContain("No head commit is recorded");
  expect(result.scope).toEqual({ kind: "pull_request", number: 80 });
});

it("states that a run has no pull request rather than answering with its branch diff", async () => {
  writeFileSync(join(worktreePath, "notes.md"), "alpha\n");

  const result = await review.getDiff(TARGET, PULL_REQUEST);

  expect(result.diff).toBeNull();
  expect(result.unavailable).toContain("no pull request");
  expect(result.scope).toEqual({ kind: "pull_request", number: null });
});

it("names the branch and the base ref its diff spans, and pins a head that holds uncommitted work", async () => {
  writeFileSync(join(worktreePath, "feature.md"), "branch work\n");

  const result = await review.getDiff(TARGET, BRANCH_DIFF_SCOPE);
  commit("feature");

  expect(result.scope).toEqual({ kind: "branch", branch: BRANCH, base_ref: "main" });
  expect(result.diff?.base).toBe(fix.repo.git("rev-parse", "main").trim());
  expect(result.diff?.head).toBe(git("rev-parse", "HEAD^{tree}"));
});

it("measures the branch against the branch its pull request targets, not against its fork base", async () => {
  fix.repo.git("branch", "release", "main");
  fix.repo.write("after-release.md", "only on main\n");
  fix.repo.commitAll("main moves past release");
  git("rebase", "main");
  writeFileSync(join(worktreePath, "feature.md"), "branch work\n");
  commit("feature");

  const againstFork = await review.getDiff(TARGET, BRANCH_DIFF_SCOPE);
  insertRunPullRequest({ id: "pr-retargeted", number: 81, base_ref: "release" });
  const againstTarget = await review.getDiff(TARGET, BRANCH_DIFF_SCOPE);

  expect(againstFork.scope).toMatchObject({ base_ref: "main" });
  expect(againstFork.diff?.files.map((file) => file.path)).toEqual(["feature.md"]);
  expect(againstTarget.scope).toMatchObject({ base_ref: "release" });
  expect(againstTarget.diff?.base).toBe(fix.repo.git("rev-parse", "release").trim());
  expect(againstTarget.diff?.files.map((file) => file.path)).toEqual([
    "after-release.md",
    "feature.md",
  ]);
});

it("counts a scope's own files and lines, never another scope's", async () => {
  await capture(false);
  writeFileSync(join(worktreePath, "first.md"), "one\n");
  commit("first");
  await capture(true);
  writeFileSync(join(worktreePath, "second.md"), "two\nthree\n");
  const only = commit("second");

  const branchDiff = (await review.getDiff(TARGET, BRANCH_DIFF_SCOPE)).diff;
  const step = (await review.getDiff(TARGET, STEP)).diff;
  const one = (await review.getDiff(TARGET, { kind: "commit", commit: only })).diff;

  expect([branchDiff?.files.length, branchDiff?.additions]).toEqual([2, 3]);
  expect([step?.files.length, step?.additions]).toEqual([1, 1]);
  expect([one?.files.length, one?.additions]).toEqual([1, 2]);
});

it("answers a step that left the tree it entered on with an empty diff, not with a refusal", async () => {
  await capture(false);
  await capture(true);

  const result = await review.getDiff(TARGET, STEP);

  expect(result.unavailable).toBeNull();
  expect(result.diff?.files).toEqual([]);
  expect(result.diff?.additions).toBe(0);
});

it("answers a commit that changed nothing with an empty diff, not with a refusal", async () => {
  git("commit", "--allow-empty", "-m", "empty");

  const result = await review.getDiff(TARGET, { kind: "commit", commit: git("rev-parse", "HEAD") });

  expect(result.unavailable).toBeNull();
  expect(result.diff?.files).toEqual([]);
  expect(result.scope).toMatchObject({ kind: "commit", subject: "empty" });
});

it("refuses a step of another run rather than answering with this one's", async () => {
  seedWorkflowRun(fix.db, {
    runId: "r-other",
    repositoryId: "repo-1",
    runStatus: "review_ready",
    steps: [{ id: "s-other", name: "Plan", status: "succeeded" }],
  });

  await expect(review.getDiff(TARGET, { kind: "step", step: "s-other" })).rejects.toThrow(
    DiffScopeNotFoundError,
  );
  await expect(review.getDiff(TARGET, { kind: "step", step: "nope" })).rejects.toThrow(
    DiffScopeNotFoundError,
  );
});
