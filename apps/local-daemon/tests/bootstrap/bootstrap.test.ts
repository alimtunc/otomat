...
it("freezes the default project's root once it owns a repository, whatever the boot root is", async () => {
  // The production order: the boot also materializes the repository, which binds the root.
  await ensureDefaultRepository(t.db, ensureDefaultProject(t.db, repo.root));
...
it("keeps a repository registered onto the default project when the daemon reboots", async () => {
  // The user booted outside a git repository, then registered one through the blocked launch.
  await ensureDefaultRepository(t.db, ensureDefaultProject(t.db, scratch));
  expect(getRepository(t.db, DEFAULT_REPOSITORY_ID)).toBeUndefined();

  const registered = await registerLocalRepository(t.db, repo.root, DEFAULT_PROJECT_ID);
...
it("creates the default repository row with the detected branch and refreshes it across boots", async () => {
  const projectId = ensureDefaultProject(t.db, repo.root);
  await ensureDefaultRepository(t.db, projectId);
...
it("writes no repository row for a non-git boot root", async () => {
...
it("reuses the registered project's repository when booting from a registered root", async () => {
  t.db
    .insert(schema.projects)
    .values({ id: "registered", name: "R", root_path: realpathSync(repo.root) })
    .run();
